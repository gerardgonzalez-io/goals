...
struct StudySessionTests
{
    ...
    func normalizedDaySameCalendarDay() {
        ...
        let s1 = makeSession(topic: topic, start: TestDates.date(2025, 10, 12, 9, 15), minutes: 10)
        let s2 = makeSession(topic: topic, start: TestDates.date(2025, 10, 12, 23, 59), minutes: 1)
    ...
    func durationFloors(minutes: Int, extraSeconds: Int, expected: Int) {
        ...
        let s = makeSession(topic: topic, start: start, minutes: minutes, extraSeconds: extraSeconds)
    ...
    func durationNonPositiveIsZero() {
        ...
        let s1 = StudySession(topic: topic, startDate: start, endDate: start)
        #expect(s1.durationInMinutes == 0)

        let s2 = StudySession(topic: topic, startDate: start, endDate: start.addingTimeInterval(-10))
